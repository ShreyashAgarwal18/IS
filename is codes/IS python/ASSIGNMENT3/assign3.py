...
def feistel(right32, key48):
    expanded = permute(right32, E)
    x = xor(expanded, key48)
    sbox_out = sbox_substitution(x)
...
def des_encrypt(plaintext64, round_keys):
    ...
    for i in range(16):
        temp = right
        f_out = feistel(right, round_keys[i])
...
def des_decrypt(cipher64, round_keys):
    return des_encrypt(cipher64, round_keys[::-1])
...
#Enter 64-bit binary key:
#1010101010111011000010010001100000100111001101101100110011011101

#Enter 64-bit binary plaintext:
#1101011100101000001000100110100001000001100101101000000110011010
